...
describe("formatRunDuration", () => {
  it("reports bare seconds under a minute", () => {
    ...
    expect(formatRunDuration(1_000)).toBe("1秒");
    expect(formatRunDuration(15_000)).toBe("15秒");
    expect(formatRunDuration(59_999)).toBe("59秒");
  });

  it("switches to minutes and pads the seconds", () => {
    expect(formatRunDuration(60_000)).toBe("1分00秒");
    expect(formatRunDuration(65_000)).toBe("1分05秒");
    expect(formatRunDuration(125_000)).toBe("2分05秒");
    expect(formatRunDuration(600_000)).toBe("10分00秒");
  });

  it("truncates rather than rounds, so the clock never reads ahead", () => {
    expect(formatRunDuration(1_999)).toBe("1秒");
    expect(formatRunDuration(59_999)).toBe("59秒");
    expect(formatRunDuration(60_999)).toBe("1分00秒");
  });
...
describe("formatTurnDuration", () => {
  it("keeps one decimal below a minute, as dsh's compactSeconds does", () => {
    expect(formatTurnDuration(42_000)).toBe("42秒");
    expect(formatTurnDuration(1_400)).toBe("1.4秒");
    expect(formatTurnDuration(45_200)).toBe("45.2秒");
    expect(formatTurnDuration(0)).toBe("0秒");
  });

  it("switches to minutes and seconds at 60s", () => {
    expect(formatTurnDuration(60_000)).toBe("1分0秒");
    expect(formatTurnDuration(162_000)).toBe("2分42秒");
    // 59.6s is still under the minute, so it stays in the decimal branch —
    // the same boundary dsh's compactSeconds uses.
    expect(formatTurnDuration(59_600)).toBe("59.6秒");
  });
});
...
describe("formatMessageTime", () => {
  it("renders dsh's `9月17日 21:00` shape with zero-padded clock parts", () => {
    ...
    expect(formatMessageTime(stamp)).toBe("9月17日 21:00");
    const early = new Date(2026, 0, 3, 9, 5).getTime();
    expect(formatMessageTime(early)).toBe("1月3日 09:05");
  });
});

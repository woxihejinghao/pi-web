...
**会话行那枚状态点是 dsh 的 `SessionStatusDots`，三档按 dsh 的优先级排：等待回答 > 进行中 > 已完成。** 有扩展正阻塞在这个会话上（`pendingUiRequests` 按会话路径命中）时先亮 warning 橙点，读屏文案「等待回答」—— dsh 的 `sessionStatuses` 也是先看待回答、再看活动；运行中共用 dsh 的 `StateDot`，但 `ongoing` 不是圆点，而是 10×10 视口里 8 个 2×2 方格绕成一圈、中间空着，逐格点亮（`dsh-state-dot-chase`，每格 125ms、1s 一循环，8 × 125ms 刚好一圈），颜色恒为品牌蓝 `--dsw-static-deepseek-450` —— dsh 把「工作中」排除在状态色之外，它不该读成告警；运行结束落成普通圆点（success 绿）。后两档按会话路径记：`agent_start` 点亮，`agent_settled` 时若用户不在看这个会话就留一枚「已完成」，打开会话即清除（运行中的标记不清），`session_closed`（进程被关）也清掉，否则点阵会一直转下去。三档都没有的空闲会话不显示任何标记，保持 dsh 那条「左边缘干净」的规则；圆点只出现在有状态的行里，所以列表不会因为一个永远不会再出现的槽位而整体右移。

**「已完成」还有一枚浏览器级的对应物，但它只对应「完成」这半边。** 通用设置里的「浏览器通知」（`browserNotifications`，默认关闭）打开后，`agent_settled` 且用户不在看该会话时发一条系统通知，标题是会话名、正文一句「会话任务已完成」，点击跳回该会话。**不通知开始**：任务是自己派出去的，开始不是一个需要把用户叫回来的消息；会走到通知这一步的只有「跑完了，可以来看结果」。默认关闭是因为权限提示只能由用户手势触发——页面加载时弹权限会被所有现代浏览器直接忽略——所以开关本身就是权限的申请入口：打开即 `requestPermission()`，拿不到 `granted` 就把开关弹回去并给一句提示，**绝不把偏好写进 store**，否则刷新之后开关会替一个永远不会出现的通知做担保。反过来说，权限被用户在浏览器设置里事后撤销时，这里不去自动改写偏好，`showNotification` 自己按 `permission` 静默跳过：用户没改变过选择，替他改是另一种撒谎。通知只在**本标签页看见过它开始**时才发（`sessionActivity` 里得有 `ongoing`）：刷新会让运行态一起丢掉，此时 `agent_settled` 无法区分「本来就在跑」和「刚跑完」，而侧栏那枚 done 点宁可按「提醒去看看」多标一次，通知不能同样处理——凭空报一次完成比少报一次更糟。`tag` 用会话路径，同一会话的第二次完成替换掉第一条，而不是把同一份消息叠成一摞。

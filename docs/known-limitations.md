...
- 变更面板**自动跟随工作区文件变化**（服务端递归监听工作树，`node_modules` 除外），不再需要手动刷新；但**不做提交详情、任意两个 revision 的对比、建分支/合并/rebase，也没有并排（side-by-side）diff 视图**。
...
- **图片以 base64 进会话文件**：`data` 不落盘到别处，所以一张 5MB 的截图会把那条消息撑大 6.7MB 左右，而这个会话之后的每次磁盘读取都要带上它（`buildSessionContext()` 不裁剪内容块，只按 compaction 走树）。读取时会在浏览器里缩放（长边 ≤ 1568px）并重新编码，截图和照片通常因此小一个数量级；上限仍是 8 张/条、5MB/张（按压缩前的文件算），没有对整条消息的总量再设一道闸。
- **图片只认 `png/jpeg/gif/webp`**，`svg` 会被明确拒绝（它是文档而不是图片）。压缩只做缩放 + 重编码：**GIF 不压缩**（重绘会只剩第一帧），已经在 1568px 以内且小于 512KB 的图也原样放行；解码失败、画布不可用、或重编码反而更大时一律回退原文件，不会因为压缩丢图。
